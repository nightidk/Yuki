...
exports.default = new DotCommand_1.DotCommand({
    ...
    run: async ({ client, message, mentions }) => {
        if (mentions.length == 0)
            return;
        const userDB = await (0, DatabaseFunctions_1.getUser)(message.author.id);
        if (!userDB.marry.married)
            return;
        if (userDB.marry.partner !== mentions.at(0).id)
            return;
        const image = await (0, ReactionFunctions_1.getRandomImage)("kiss");
        if (mentions.length == 0)
            await message.reply({
                embeds: [
            ...
            const msg = await message.reply({
                embeds: [
                    new Embed_1.ExtendedEmbed()
                        .setDescription(`${mentions.at(0)}, тебя хочет поцеловать ${message.member}. Что ответишь?`)
                        .setAuthor({ name: "Реакция - поцелуй" })
                        .setFooter({
                        text: message.member.displayName,
                        iconURL: message.member.displayAvatarURL({
