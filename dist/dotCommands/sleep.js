...
exports.default = new DotCommand_1.DotCommand({
    ...
    run: async ({ client, message, mentions }) => {
        const image = await (0, ReactionFunctions_1.getRandomImage)("sleep");
        if (mentions.length == 0)
            ...
            const msg = await message.reply({
                embeds: [
                    new Embed_1.ExtendedEmbed()
                        .setDescription(`${mentions.at(0)}, с тобой хочет уснуть ${message.member}. Что ответишь?`)
                        .setAuthor({ name: "Реакция - спать" })
                        .setFooter({
                        text: message.member.displayName,
                        iconURL: message.member.displayAvatarURL({
                ...
                switch (i.customId) {
                    case "yes":
                        await msg.edit({
                            embeds: [
                                new Embed_1.ExtendedEmbed()
                                    .setDescription(`${message.author} и ${mentions.at(0)} наелись и спят.`)
                                    .setImage(image)
                                    .setAuthor({
                                    name: "Реакция - спать",
                                })
