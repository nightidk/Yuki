...
exports.default = new Command_1.Command({
    ...
    options: [
        new discord_js_1.SlashCommandUserOption()
            .setName("user")
            .setDescription("Пользователь, профиль которого ты хочешь посмотреть")
            .setRequired(false),
    ],
    run: async ({ interaction, args }) => {
        const member = args.getMember("user") || interaction.member;
        const userDB = await (0, DatabaseFunctions_1.getUser)(member.id);
        if (!member.roles.cache.has("1125888088456040580") &&
            ...
            if (member.id === interaction.member.id)
                return await interaction.reply({
                    ephemeral: true,
                    embeds: [
                        new Embed_1.ExtendedEmbed().setDescription("У тебя не найдена роль фракции! Пожалуйста, обратись к администратору для выдачи."),
                    ],
                });
            else
                return await interaction.reply({
                    ephemeral: true,
                    embeds: [
                        new Embed_1.ExtendedEmbed().setDescription("У данного пользователя не найдена роль фракции!"),
                    ],
                });
        const msg = await interaction.reply({
            embeds: [
                new Embed_1.ExtendedEmbed()
                    .setDescription("Система генерирует профиль. Пожалуйста, подождите немного...")
                    .setFooter({
        ...
        (0, canvas_1.registerFont)(`${__dirname}/../../fonts/ProximaNova-Bold.ttf`, {
            family: "Proxima Nova Bold",
        });
        (0, canvas_1.registerFont)(`${__dirname}/../../fonts/lcdnova.ttf`, {
            family: "a_LCDNova",
        });
        ...
        let background;
        if (member.roles.cache.has("1125888088456040580"))
            background = await (0, canvas_1.loadImage)(`${__dirname}/../../assets/profile/hub.png`);
        else if (member.roles.cache.has("1125108934915653703"))
            background = await (0, canvas_1.loadImage)(`${__dirname}/../../assets/profile/4hikari.png`);
        else if (member.roles.cache.has("1125109766222524537"))
            background = await (0, canvas_1.loadImage)(`${__dirname}/../../assets/profile/sin.png`);
        else if (member.roles.cache.has("1125109853136896090"))
            background = await (0, canvas_1.loadImage)(`${__dirname}/../../assets/profile/tansu.png`);
        else if (member.roles.cache.has("1125110023144624178"))
            background = await (0, canvas_1.loadImage)(`${__dirname}/../../assets/profile/ken_getto.png`);
        ctx.drawImage(background, 0, 0);
        const memberAvatar = await (0, CommandFunctions_1.getAvatar)(member.displayAvatarURL({
            forceStatic: true,
            extension: "png",
            size: 256,
        }));
        ...
        if (!userDB.marry.married) {
            ctx.drawImage(await (0, canvas_1.loadImage)(`${__dirname}/../../assets/profile/modules/block_heart.png`), 837, 837);
            ...
            const partner = await interaction.guild.members
                ...
                .catch(() => { });
            if (partner) {
                const partnerAvatar = await (0, CommandFunctions_1.getAvatar)(partner.displayAvatarURL({
                    forceStatic: true,
                    extension: "png",
                    size: 128,
                }));
                ...
                ctx.font = "41px Proxima Nova Bold";
                ctx.fillStyle = "rgba(255, 255, 255, 0.33)";
                ctx.fillText(partner.user.username.length > 20
                    ? `${partner.user.username.slice(0, 20)}...`
                    : partner.user.username, 750, 864);
                const days = `${Math.floor((new Date().getTime() - userDB.marry.marryDate.getTime()) /
                    1000 /
                    (3600 * 24))} дн.`;
                ...
                ctx.font = "41px Proxima Nova Bold";
                ctx.fillStyle = "rgba(255, 255, 255, 0.33)";
                ctx.fillText(userDB.marry.typeMarry === "dating" ? "Dating" : "Married", 732, 747);
                ctx.fillStyle = "rgba(255, 255, 255, 0.61)";
                const date = (0, CommandFunctions_1.getStringDate)(userDB.marry.marryDate);
        ...
        ctx.drawImage(await (0, canvas_1.loadImage)(`${__dirname}/../../assets/profile/modules/block_guild.png`), 1502, 837);
        ...
        ctx.fillText((0, CommandFunctions_1.getTimeBalance)(userDB.balance), 1391, 408);
        ctx.font = "53px Proxima Nova Bold";
        ctx.fillStyle = "rgba(255, 255, 255, 0.62)";
        ctx.fillText(member.user.username.length > 20
            ? `${member.user.username.slice(0, 20)}...`
            : member.user.username, 535, 271);
        ...
        (0, CommandFunctions_1.roundRect)(ctx, 535, 347, Number((586 *
            (Number((100 / ((0, CommandFunctions_1.getLevelExp)(level) / userDB.level.xp)).toFixed(2)) /
                100)).toFixed(2)), 17, 12.5);
        if (member.roles.cache.has("1125888088456040580"))
        ...
        ctx.font = "30px Proxima Nova Bold";
        ctx.fillStyle = "rgba(255, 255, 255, 0.62)";
        const timeInVoice = (0, CommandFunctions_1.getTime)(userDB.stats.voiceActive.all);
        ...
        await msg.edit({
            embeds: [],
            files: [
                new discord_js_1.AttachmentBuilder(canvas.toBuffer(), {
